...
const InstitutionAcademicLevelManager = () => {
    ...
    const fetchInstitutions = async () => {
      try {
        const res = await axios.get("http://localhost:8080/admin/institutions/dto");
    ...
    const fetchRelations = async () => {
      try {
        const res = await axios.get("http://localhost:8080/admin/institution-academic-levels");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {      
        await axios.post("http://localhost:8080/admin/institution-academic-levels", null, {
          params: {
    ...
    const handleDelete = async (id: number) => {
      ...
      try {
        await axios.delete(`http://localhost:8080/admin/institution-academic-levels/${id}`);
        fetchRelations();
      } catch (err) {
  ...
  return (
    <div className="mt-10">
      <h2 className="text-2xl font-bold mb-4">Asignar Niveles Académicos a Instituciones</h2>
      ...
      <table className="min-w-full divide-y divide-gray-200 shadow-sm border border-gray-200 rounded-lg overflow-hidden">
        <thead className="bg-blue-600 text-white">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-sm font-semibold">Nivel Académico</th>
            <th className="px-4 py-3 text-center text-sm font-semibold">Acciones</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-100">
          {relations.map((rel) => (
            <tr key={rel.id} className="hover:bg-gray-50 transition-colors duration-200">
